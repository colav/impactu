# openalex
```
openalex> db.works.distinct("type")
[
  'article',
  'book',
  'book-chapter',
  'dataset',
  'dissertation',
  'editorial',
  'erratum',
  'grant',
  'letter',
  'libguides',
  'other',
  'paratext',
  'peer-review',
  'preprint',
  'reference-entry',
  'report',
  'retraction',
  'review',
  'standard',
  'supplementary-materials'
]

```

# Google Scholar

```
{'@article',
 '@book',
 '@incollection',
 '@inproceedings',
 '@mastersthesis',
 '@misc',
 '@phdthesis',
 '@techreport'}
```

# Datos Abierto de Minciencias DAM
Global Types
```
['Apropiación social del conocimiento',
 'Apropiación social del conocimiento y divulgación pública de la ciencia',
 'Desarrollo tecnológico e innovación',
 'Formación de recurso humano',
 'No categorizado',
 'Nuevo conocimiento']
```

Types and Subtypes

```
Apropiación social del conocimiento
*  Artículos
*  Boletín divulgativo de resultado de investigación
*  Capítulos de libro
*  Consultoría científico-técnica
*  Consultoría en artes, arquitectura y diseño
*  Documento de trabajo
*  Edicion
*  Espacios de participación ciudadana
*  Estrategia de comunicación del conocimiento
*  Estrategias pedagógicas para el fomento a la CTI
*  Evento científico
*  Eventos artísticos
*  Generación de contenido de audio
*  Generación de contenido impresa
*  Generación de contenido multimedia
*  Generación de contenido virtual
*  Informe Final de Investigación
*  Libros
*  Notas científica
*  Nueva secuencia genética
*  Participación ciudadana en proyectos de CTI
*  Red de conocimiento especializado
*  Talleres de creación



Apropiación social del conocimiento y divulgación pública de la ciencia
*  Artículos
*  Boletín divulgativo de resultado de investigación
*  Consultoría científico-técnica
*  Consultoría en artes, arquitectura y diseño
*  Desarrollos Web
*  Documento de trabajo
*  Edicion
*  Evento científico
*  Eventos artísticos, de arquitectura o de diseño con componentes de apropiación
*  Informe Final de Investigación
*  Informe técnico final
*  Libros de Divulgación de investigación y/o Compilación de Divulgación
*  Libros de Formación
*  Manuales y Guías Especializadas
*  Notas científica
*  Nueva secuencia genética
*  Procesos de Apropiación social del conocimiento para el fortalecimiento de cadenas productivas
*  Procesos de apropiación resultado del trabajo conjunto entre un Centro de Ciencia y un grupo
*  Procesos de apropiación social del conocimiento para el fortalecimiento o solución de asuntos
*  Procesos de apropiación social del conocimiento para la generación de insumos
*  Producciones de contenido digital
*  Producción de estrategias y contenidos Transmedia
*  Publicaciones editoriales no especializadas
*  Red de conocimiento especializado
*  Talleres de creación



Desarrollo tecnológico e innovación
*  Colección científica
*  Conceptos técnicos
*  Consultorias
*  Consultoría científico-técnica
*  Consultoría en artes, arquitectura y diseño
*  Diseño Industrial
*  Empresa de base tecnológica
*  Empresas creativas y culturales
*  Esquema de circuito integrado
*  Informe Final de Investigación
*  Informe técnico final
*  Innovacion de gestion empresarial
*  Innovaciones en procedimiento y servicio
*  Innovación de procedimiento
*  Nuevo registro científico
*  Planta Piloto
*  Productos nutraceúticos
*  Prototipo Industrial
*  Registros de acuerdos de licencia para la explotación de obras
*  Regulación Norma o Reglamento
*  Regulación y norma
*  Regulación y norma acto legislativo
*  Regulación y norma manejo clínico forense
*  Regulación y norma manuales y modelos de atención diferencial a víctimas
*  Regulación y norma protocolos de atención a usuarios/víctimas (pacientes)
*  Regulación y norma protocolos de vigilancia epidemiológica
*  Regulación y norma proyecto de ley
*  Regulación y norma práctica clínica
*  Secreto empresarial
*  Signos Distintivos
*  Software
*  Spin-off



Formación de recurso humano
*  Apoyo creación de cursos
*  Apoyo de programas
*  Asesoría al programa Ondas
*  Proyecto ID+I con Formación
*  Proyecto de Investigacion y Desarrollo
*  Proyecto de Investigación y Creación
*  Proyecto de extensión
*  Proyecto de extensión y responsabilidad social en CTI
*  Tesis de doctorado
*  Tesis de maestria
*  Tesis de pregrado



No categorizado
*  Registro general



Nuevo conocimiento
*  Artículos
*  Artículos de investigación
*  Capítulos de libro
*  Capítulos de libro de investigación
*  Libros
*  Libros de Formación
*  Libros de investigación
*  Notas científica
*  Obras o productos de arte, arquitectura y diseño
*  Patente de invención
*  Patente modelo de utilidad
*  Variedad animal
*  Variedad vegetal
```


